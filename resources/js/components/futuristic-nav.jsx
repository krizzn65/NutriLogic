...
const LumaBar = () => {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
      <div
        className="relative flex items-center justify-center gap-6 bg-white/20 dark:bg-black/20 backdrop-blur-2xl rounded-full px-6 py-3 shadow-xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
        ...
        {items.map((item, index) => {
          ...
          return (
            <motion.div key={item.id} className="relative flex flex-col items-center group">
              ...
              {/* Tooltip */}
              <span
                className="absolute bottom-full mb-2 px-2 py-1 text-xs rounded-md bg-gray-500 text-white dark:bg-gray-200 dark:text-black opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                {item.label}
              </span>
            </motion.div>
          );
