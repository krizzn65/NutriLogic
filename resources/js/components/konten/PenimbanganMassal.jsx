...
const OFFLINE_WEIGHING_SNAPSHOT_KEY =
    "kader_offline_weighing_children_snapshot";
...
export default function KegiatanPosyandu() {
    ...
    const [pendingOfflineWeighingQueue, setPendingOfflineWeighingQueue] =
        useState([]);
    const [syncingOfflineQueue, setSyncingOfflineQueue] = useState(false);
    const [offlineSyncSummary, setOfflineSyncSummary] = useState(null);
    ...
    const [vitaminDate, setVitaminDate] = useState(
        new Date().toISOString().split("T")[0],
    );
    const [vitaminPickerDate, setVitaminPickerDate] = useState(new Date());
    const [isVitaminDatePickerOpen, setIsVitaminDatePickerOpen] =
        useState(false);
    const vitaminDateButtonRef = useRef(null);
    const [vitaminDropdownPos, setVitaminDropdownPos] = useState({
        top: 0,
        left: 0,
    });
    ...
    const [immunizationPickerDate, setImmunizationPickerDate] = useState(
        new Date(),
    );
    const [isImmunizationDatePickerOpen, setIsImmunizationDatePickerOpen] =
        useState(false);
    const immunizationDateButtonRef = useRef(null);
    const [immunizationDropdownPos, setImmunizationDropdownPos] = useState({
        top: 0,
        left: 0,
    });
    ...
    const toggleImmunizationDatePicker = () => {
        if (
            !isImmunizationDatePickerOpen &&
            immunizationDateButtonRef.current
        ) {
            const rect =
                immunizationDateButtonRef.current.getBoundingClientRect();
    ...
    const createInitialWeighingData = (childrenData) => {
        const initialData = {};
            ...
            if (child.today_weighing) {
                initialData[child.id] = {
                    ...
                    head_circumference_cm:
                        child.today_weighing.head_circumference_cm || "",
    ...
    useEffect(() => {
        const initialQueue = readStorage(OFFLINE_WEIGHING_QUEUE_KEY, []);
        setPendingOfflineWeighingQueue(
    ...
    const fetchChildren = async (forceRefresh = false) => {
        ...
        if (!forceRefresh) {
            ...
            if (cachedChildren) {
                ...
                setWeighingData(createInitialWeighingData(cachedChildren));
                setLoading(false);
                return;
            }
        }
        ...
        if (!navigator.onLine) {
            const offlineSnapshot = readStorage(
                OFFLINE_WEIGHING_SNAPSHOT_KEY,
                [],
            );
            if (Array.isArray(offlineSnapshot) && offlineSnapshot.length > 0) {
                setChildren(offlineSnapshot);
                setWeighingData(createInitialWeighingData(offlineSnapshot));
                setLoading(false);
                setError(null);
                setOfflineSyncSummary({
                    type: "info",
                    message:
                        "Mode offline aktif. Menampilkan data anak terakhir yang tersimpan di perangkat.",
                });
                return;
            }
        }
        ...
        try {
            ...
            writeStorage(OFFLINE_WEIGHING_SNAPSHOT_KEY, childrenData);
            setWeighingData(createInitialWeighingData(childrenData));
        } catch (err) {
            const offlineSnapshot = readStorage(
                OFFLINE_WEIGHING_SNAPSHOT_KEY,
                [],
            ...
            if (Array.isArray(offlineSnapshot) && offlineSnapshot.length > 0) {
                ...
                setOfflineSyncSummary({
                    type: "warning",
                    message:
                        "Gagal memuat data terbaru. Menggunakan data lokal terakhir.",
                });
            } else {
                const errorMessage =
    ...
    const updateWeighing = async (child) => {
        ...
        try {
            ...
            successDiv.className =
                "fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 animate-in fade-in slide-in-from-top-2 flex items-center gap-2";
    ...
    const syncOfflineWeighingQueue = async () => {
        if (!navigator.onLine || syncingOfflineQueue) return;

        const currentQueue = readStorage(OFFLINE_WEIGHING_QUEUE_KEY, []);
        ...
        try {
            ...
            for (const entry of currentQueue) {
                try {
                    ...
                    await api.post("/kader/weighings/bulk", {
                        weighings: entry.weighings,
                    });
                    syncedRecordsCount += entry.weighings.length;
                } catch (syncError) {
                    const shouldRetryLater =
                        !syncError.response || syncError.response.status >= 500;
                    if (shouldRetryLater) {
            ...
            writeStorage(OFFLINE_WEIGHING_QUEUE_KEY, remainingQueue);
            setPendingOfflineWeighingQueue(remainingQueue);

            if (syncedRecordsCount > 0) {
                setOfflineSyncSummary({
                    type: "success",
                    message: `${syncedRecordsCount} data offline berhasil disinkronkan.`,
                });
                ...
                setOfflineSyncSummary({
                    type: "warning",
                    message:
                        "Sebagian data offline belum bisa disinkronkan. Sistem akan mencoba lagi saat koneksi stabil.",
                });
            }
        } finally {
    ...
    useEffect(() => {
        if (isOnline && pendingOfflineWeighingQueue.length > 0) {
            syncOfflineWeighingQueue();
        }
    }, [isOnline, pendingOfflineWeighingQueue.length]);
    ...
    const handleSubmit = async (e) => {
            ...
            if (data.weight_kg && data.height_cm) {
                weighings.push({
                    ...
                    head_circumference_cm: data.head_circumference_cm
                        ? parseFloat(data.head_circumference_cm)
                        : null,
                    notes: data.notes || null,
                });
            }
        ...
        if (!isOnline) {
            const currentQueue = readStorage(OFFLINE_WEIGHING_QUEUE_KEY, []);
            const updatedQueue = [
                ...(Array.isArray(currentQueue) ? currentQueue : []),
                {
            ...
            writeStorage(OFFLINE_WEIGHING_QUEUE_KEY, updatedQueue);
            setPendingOfflineWeighingQueue(updatedQueue);
            clearWeighingForm();
            setResults(null);
            setWarnings(null);
            setOfflineSyncSummary({
                type: "info",
                message: `${weighings.length} data disimpan offline. Akan disinkronkan otomatis saat online.`,
            });
            return;
        }
        ...
        try {
            const response = await api.post("/kader/weighings/bulk", {
                weighings,
            });
    ...
    const updateVitamin = async (child) => {
        ...
        try {
            ...
            const response = await api.put(
                `/kader/vitamins/${child.today_vitamin.id}`,
                {
    ...
    const handleVitaminSubmit = async (e) => {
        ...
        try {
            const response = await api.post("/kader/vitamins/bulk", {
                distributions,
            });
    ...
    const updateImunisasi = async (child) => {
        ...
        try {
            setSubmitting(true);
            const response = await api.put(
                `/kader/immunizations/${child.today_immunization.id}`,
                {
    ...
    const handleImmunizationSubmit = async (e) => {
        ...
        try {
            const response = await api.post("/kader/immunizations/bulk", {
                records,
            });
    ...
    return (
        ...
        >
            ...
            {activeTab === "penimbangan" && !isOnline && (
                <div className="mb-4 bg-amber-50 border border-amber-200 text-amber-800 px-4 py-3 rounded-xl flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-amber-500 shrink-0"></span>
                    <span className="text-sm font-medium">
                        Mode offline aktif. Data penimbangan akan disimpan lokal
            ...
            {activeTab === "penimbangan" &&
                pendingOfflineWeighingQueue.length > 0 && (
                    <div className="mb-4 bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <p className="text-sm font-medium">
                            Ada{" "}
                            <span className="font-bold">
                                {pendingOfflineWeighingQueue.length}
                            </span>{" "}
                            batch data offline yang menunggu sinkronisasi.
                        </p>
                        <button
                            type="button"
                            onClick={syncOfflineWeighingQueue}
                            disabled={!isOnline || syncingOfflineQueue}
                            className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed"
                        >
                            {syncingOfflineQueue
                                ? "Menyinkronkan..."
                                : "Sync Sekarang"}
            ...
            {offlineSyncSummary && (
                <div
                    className={`mb-4 px-4 py-3 rounded-xl text-sm font-medium ${
                        offlineSyncSummary.type === "success"
                            ? "bg-green-50 border border-green-200 text-green-800"
                            : offlineSyncSummary.type === "warning"
                              ? "bg-yellow-50 border border-yellow-200 text-yellow-800"
                              : "bg-slate-50 border border-slate-200 text-slate-700"
                    }`}
                >
                    {offlineSyncSummary.message}
                </div>
            )}
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                    <p className="font-bold text-yellow-800 text-sm">
                                        {warnings.length} anak sudah ditimbang
                                        hari ini
                                    </p>
                        ...
                        {results.data.map((weighing) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span
                                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border mt-3 ${getStatusColor(weighing.nutritional_status)}`}
                                    >
                                        {getStatusLabel(
                        ...
                        <div className="p-5 border-b border-gray-100 flex flex-col gap-4 bg-gray-50/50 flex-shrink-0">
                            <div className="flex flex-col md:flex-row gap-4 justify-between items-end md:items-center">
                                <div className="w-full md:w-auto">
                                    <label
                                        htmlFor="penimbangan-date"
                                        className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5 block"
                                    >
                                        Tanggal Penimbangan
                                    </label>
                            ...
                            <div className="w-full">
                                ...
                                {searchQuery && (
                                    <p className="mt-1.5 text-xs text-gray-500">
                                        Menampilkan {filteredChildren.length}{" "}
                                        dari {children.length} anak
                                    </p>
                                )}
                            </div>
                        </div>
                                ...
                                <div className="md:hidden flex flex-col divide-y divide-gray-100">
                                    {filteredChildren.length === 0 ? (
                                        <div className="p-8 text-center text-gray-500">
                                            {searchQuery
                                                ? `Tidak ada anak dengan nama "${searchQuery}"`
                                        ...
                                        filteredChildren.map((child, index) => (
                                            <div
                                                key={child.id}
                                                className={`p-4 flex flex-col gap-3 border-b border-gray-100 last:border-b-0 ${child.today_weighing && !editMode[child.id] ? "bg-blue-50/40" : "bg-white"}`}
                                            >
                                                {/* Child Info */}
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <span
                                                                className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${child.gender === "L" ? "bg-blue-50 text-blue-600" : "bg-orange-50 text-orange-600"}`}
                                                            >
                                                                {child.gender ===
                                                                "L"
                                                                    ? "Laki-laki"
                                                            ...
                                                            >
                                                                <div className="bg-gray-50/80 rounded-lg p-3 border border-gray-100/50 mb-3">
                                                                    <div className="flex justify-between items-center mb-2">
                                                                        ...
                                                                        <span
                                                                            className={`text-[9px] px-1.5 py-0.5 rounded font-medium border ${getStatusColor(child.latest_weighing.nutritional_status)}`}
                                                                        >
                                                                            {getStatusLabel(
                                                                                child
                                                                                    .latest_weighing
                                                                    ...
                                                                    <div className="grid grid-cols-4 gap-2 text-center divide-x divide-gray-200">
                                                                        <div className="px-1 first:pl-0">
                                                                            <span className="block text-[9px] text-gray-400 uppercase mb-0.5">
                                                                                Berat
                                                                            </span>
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <label
                                                                htmlFor={`weighing-weight-${child.id}`}
                                                        ...
                                                        <input
                                                            ...
                                                            className={`w-full px-3 py-2 border rounded-lg transition-all text-sm font-medium placeholder:text-gray-400 ${
                                                                isInputDisabled(
                                                                    child,
                                                                )
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <label
                                                                htmlFor={`weighing-height-${child.id}`}
                                                                className="text-[10px] font-bold text-gray-500 uppercase block"
                                                            >
                                                                Tinggi (cm)
                                                            </label>
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <label
                                                                htmlFor={`weighing-muac-${child.id}`}
                                                                className="text-[10px] font-bold text-gray-500 uppercase block"
                                                            >
                                                                Lengan (cm)
                                                            </label>
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-1 mb-1">
                                                            <label
                                                                htmlFor={`weighing-head-${child.id}`}
                                                                className="text-[10px] font-bold text-gray-500 uppercase block"
                                                            >
                                                                Kepala (cm)
                                                            </label>
                                                ...
                                                <div>
                                                    <input
                                                        ...
                                                        className={`w-full px-3 py-2 border rounded-lg transition-all text-sm placeholder:text-gray-400 ${
                                                            isInputDisabled(
                                                                child,
                                                            )
                                                                ? "bg-gray-100 border-gray-200 text-gray-600 cursor-not-allowed"
                                            ...
                                            {filteredChildren.length === 0 ? (
                                                ...
                                                filteredChildren.map(
                                                    (child, index) => (
                                                        <tr
                                                            key={child.id}
                                                            className={`group transition-colors ${child.today_weighing && !editMode[child.id] ? "bg-blue-50/40 hover:bg-blue-100/50" : "hover:bg-blue-50/30"}`}
                                                        >
                                                            ...
                                                            <td
                                                                className="px-6 py-4 cursor-pointer align-top"
                                                                onClick={() =>
                                                                    toggleExpandedData(
                                                                        child.id,
                                                                    )
                                                            ...
                                                            <td className="px-6 py-4 text-center">
                                                                {child.today_weighing ? (
                                                                    <button
                                                                        ...
                                                                        className={`p-2 rounded-lg transition-colors flex items-center justify-center mx-auto shadow-sm ${
                                                                            editMode[
                                                                                child
                                                                                    .id
                                                                            ]
                            ...
                            {vitaminWarnings && vitaminWarnings.length > 0 && (
                                <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-4">
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div className="flex-1">
                                            <p className="font-bold text-yellow-800 text-sm mb-2">
                                                Peringatan:
                                            </p>
                            ...
                            <div className="flex flex-col md:flex-row gap-4 justify-between items-end md:items-center">
                                <div className="w-full md:w-auto">
                                    ...
                                    <div className="relative">
                                        ...
                                        {isVitaminDatePickerOpen &&
                                            createPortal(
                                                <>
                                                    <div
                                                        className="fixed inset-0 z-9998 bg-transparent"
                                                        onClick={() =>
                                                            setIsVitaminDatePickerOpen(
                                                                false,
                                                            )
                                                        }
                                                    />
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center justify-between mb-4">
                                                            <button
                                                                type="button"
                                                                onClick={() =>
                                                                    setVitaminPickerDate(
                                                                        new Date(
                                                                            vitaminPickerDate.setMonth(
                                                                                vitaminPickerDate.getMonth() -
                                                                                    1,
                                                                            ),
                                                                        ),
                                                                    )
                                                            ...
                                                            <span className="font-semibold text-gray-800">
                                                                {vitaminPickerDate.toLocaleDateString(
                                                                    "id-ID",
                                                                    {
                                                                        month: "long",
                                                                        year: "numeric",
                                ...
                                <div className="md:hidden flex flex-col divide-y divide-gray-100">
                                    {filteredChildren.length === 0 ? (
                                        ...
                                        filteredChildren.map((child) => (
                                            <div
                                                key={child.id}
                                                className="p-4 bg-white flex flex-col gap-3 border-b border-gray-100 last:border-b-0"
                                            >
                                                {/* Child Info */}
                                                <div className="flex justify-between items-start">
                                                    <div>
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    <div className="col-span-2">
                                                        <label
                                                            htmlFor={`vitamin-type-${child.id}`}
                                                            className="text-[10px] font-bold text-gray-500 uppercase mb-1 block"
                                                        >
                                                            Jenis Vitamin *
                                                        </label>
                                                        <select
                                                            id={`vitamin-type-${child.id}`}
                                                            disabled={isVitaminInputDisabled(
                                                                child,
                                                            )}
                                                            value={
                                                                vitaminData[
                                                            ...
                                                            className={`w-full px-3 py-2 border rounded-lg transition-all text-sm font-medium ${
                                                                isVitaminInputDisabled(
                                                                    child,
                                                                )
                                                                    ? "bg-gray-100 border-gray-200 text-gray-600 cursor-not-allowed"
                                                        ...
                                                        >
                                                            ...
                                                            <option value="vitamin_a_blue">
                                                                {" "}
                                                                Vitamin A Biru
                                                                (100.000 IU)
                                                            </option>
                                                            <option value="vitamin_a_red">
                                                                {" "}
                                                                Vitamin A Merah
                                                                (200.000 IU)
                                                            </option>
                                                    ...
                                                    <div>
                                                        <label
                                                            htmlFor={`vitamin-dosage-${child.id}`}
                                                            className="text-[10px] font-bold text-gray-500 uppercase mb-1 block"
                                                        >
                                                            Dosis
                                                        </label>
                                                        <input
                                                            id={`vitamin-dosage-${child.id}`}
                                                            type="text"
                                                            disabled={isVitaminInputDisabled(
                                                                child,
                                                            )}
                                                    ...
                                                    <div>
                                                        <label
                                                            htmlFor={`vitamin-notes-${child.id}`}
                                                            className="text-[10px] font-bold text-gray-500 uppercase mb-1 block"
                                                        >
                                                            Catatan
                                                        </label>
                                                        <input
                                                            id={`vitamin-notes-${child.id}`}
                                                            type="text"
                                                            disabled={isVitaminInputDisabled(
                                                                child,
                                                            )}
                                    ...
                                    <table className="w-full">
                                        <thead>
                                            <tr className="border-b border-gray-100 bg-gray-50/50">
                                                ...
                                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-64">
                                                    Jenis Vitamin *
                                                </th>
                                                <th className="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-32">
                                                    Dosis
                                            ...
                                            {filteredChildren.length === 0 ? (
                                                ...
                                                filteredChildren.map(
                                                    (child, index) => (
                                                        <tr
                                                            key={child.id}
                                                            className={`group transition-colors ${child.today_vitamin ? "bg-green-50/60 hover:bg-green-100/60" : "hover:bg-green-50/30"}`}
                                                        >
                                                            <td className="px-6 py-4 text-sm text-gray-500 font-medium">
                                                                {index + 1}
                                                            </td>
            ...
            {activeTab === "imunisasi" && (
                <>
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-6 animate-in fade-in slide-in-from-top-2 flex-shrink-0">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="text-lg font-bold text-blue-800">
                                    {immunizationResults.message}
                                </h3>
                            </div>

                            {immunizationWarnings &&
                                immunizationWarnings.length > 0 && (
                                    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-4">
                                        <div className="flex items-start gap-2">
                                            <svg
                                                className="w-5 h-5 text-yellow-600 shrink-0 mt-0.5"
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 flex flex-col flex-1 min-h-0">
                                            ...
                                            createPortal(
                                                <>
                                                    <div
                                                        className="fixed inset-0 z-9998 bg-transparent"
                                                        onClick={() =>
                                                            setIsImmunizationDatePickerOpen(
                                                                false,
                                                            )
                                                        }
                                                    />
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center justify-between mb-4">
                                                            <button
                                                                type="button"
                                                                onClick={() =>
                                                                    setImmunizationPickerDate(
                                                                        new Date(
                                                                            immunizationPickerDate.setMonth(
                                                                                immunizationPickerDate.getMonth() -
                                                                                    1,
                                                                            ),
                                                                        ),
                                                                    )
                                                            ...
                                                            <span className="font-semibold text-gray-800">
                                                                {immunizationPickerDate.toLocaleDateString(
                                                                    "id-ID",
                                                                    {
                                                                        month: "long",
                                                                        year: "numeric",
                                                        ...
                                                        <div className="grid grid-cols-7 gap-1">
                                                            {(() => {
                                                                const daysInMonth =
                                                                    new Date(
                                                                        immunizationPickerDate.getFullYear(),
                                                                        immunizationPickerDate.getMonth() +
                                                                            1,
                                                                        0,
                                                                    ).getDate();
                        ...
                        {/* Form Content */}
                        <form
                            onSubmit={handleImmunizationSubmit}
                            className="flex flex-col flex-1 min-h-0"
                        >
                            <div className="flex-1 overflow-y-auto">
                                {/* Mobile Card View */}
                                        ...
                                        filteredChildren.map((child) => (
                                            ...
                                            >
                                                    ...
                                                    <div className="flex flex-col items-end gap-1">
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={() =>
                                                                    imunisasiEditMode[
                                                                        ...
                                                                        : toggleImunisasiEditMode(
                                                                              child.id,
                                                                          )
                                                                }
                                                                className={`p-2 rounded-lg transition-colors mb-2 ml-auto ${
                                                                    imunisasiEditMode[
                                                                        child.id
                                                                    ]
                                                                        ? "bg-green-500 text-white hover:bg-green-600 shadow-sm"
                                                        ...
                                                        {child.latest_immunization && (
                                                            <button
                                                                type="button"
                                                                onClick={() =>
                                                                    toggleExpandedImmunizationData(
                                                                        child.id,
                                                                    )
                                                                }
                                                                className="flex items-center gap-1 text-[10px] font-medium text-blue-600 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded-lg transition-colors ml-auto"
                                                            >
                                                                {expandedImmunizationData[
                                                                    child.id
                                                                ]
                                                                    ? "Tutup"
                                                                    : "Data Terakhir"}
                                                                ...
                                                                <div className="bg-blue-50/80 rounded-lg p-3 border border-blue-100/50 mb-2">
                                                                    ...
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        <span className="text-xs font-bold text-gray-700">
                                                                            {child.latest_immunization.vaccine_type
                                                                                .replace(
                                                                                    /_/g,
                                                                                    " ",
                                                                                )
                                                ...
                                                <div className="grid grid-cols-1 gap-2">
                                                    <div>
                                                        <label
                                                            htmlFor={`immunization-vaccine-${child.id}`}
                                                            className="text-[10px] font-bold text-gray-500 uppercase mb-1 block"
                                                        >
                                                            Jenis Vaksin *
                                                        </label>
                                                        <select
                                                            id={`immunization-vaccine-${child.id}`}
                                                            value={
                                                                immunizationData[
                                                                    child.id
                                                                ]
                                                            ...
                                                            disabled={isImunisasiInputDisabled(
                                                                child,
                                                            )}
                                                            className={`w-full px-3 py-2 border rounded-lg transition-all text-sm font-medium ${
                                                                isImunisasiInputDisabled(
                                                                    child,
                                                                )
                                                        ...
                                                        >
                                                            ...
                                                            <optgroup label="BCG & Hepatitis B">
                                                                ...
                                                                <option value="hepatitis_b_0">
                                                                    {" "}
                                                                    Hepatitis B
                                                                    0 (HB0)
                                                                </option>
                                                                <option value="hepatitis_b_1">
                                                                    {" "}
                                                                    Hepatitis B
                                                                    1
                                                                </option>
                                                                <option value="hepatitis_b_2">
                                                                    {" "}
                                                                    Hepatitis B
                                                                    2
                                                                </option>
                                                                <option value="hepatitis_b_3">
                                                                    {" "}
                                                                    Hepatitis B
                                                                    3
                                                                </option>
                                                            ...
                                                            <optgroup label="Polio">
                                                                <option value="polio_0">
                                                                    {" "}
                                                                    Polio 0
                                                                </option>
                                                                <option value="polio_1">
                                                                    {" "}
                                                                    Polio 1
                                                                </option>
                                                                <option value="polio_2">
                                                                    {" "}
                                                                    Polio 2
                                                                </option>
                                                                <option value="polio_3">
                                                                    {" "}
                                                                    Polio 3
                                                                </option>
                                                                <option value="polio_4">
                                                                    {" "}
                                                                    Polio 4
                                                                </option>
                                                            </optgroup>
                                                            <optgroup label="DPT-HiB-HepB (Pentavalent)">
                                                                <option value="dpt_hib_hep_b_1">
                                                                    {" "}
                                                                    DPT-HiB-HepB
                                                                    1
                                                                </option>
                                                                <option value="dpt_hib_hep_b_2">
                                                                    {" "}
                                                                    DPT-HiB-HepB
                                                                    2
                                                                </option>
                                                                <option value="dpt_hib_hep_b_3">
                                                                    {" "}
                                                                    DPT-HiB-HepB
                                                                    3
                                                                </option>
                                                            ...
                                                            <optgroup label="IPV & Campak-Rubella">
                                                                ...
                                                                <option value="campak_rubella_1">
                                                                    {" "}
                                                                    Campak-Rubella
                                                                    1 (MR1)
                                                                </option>
                                                                <option value="campak_rubella_2">
                                                                    {" "}
                                                                    Campak-Rubella
                                                                    2 (MR2)
                                                                </option>
                                                    ...
                                                    <div>
                                                        <label
                                                            htmlFor={`immunization-notes-${child.id}`}
                                                            className="text-[10px] font-bold text-gray-500 uppercase mb-1 block"
                                                        >
                                                            Catatan
                                                        </label>
                                                        <input
                                                            id={`immunization-notes-${child.id}`}
                                                            type="text"
                                                            value={
                                                                immunizationData[
                                                                    child.id
                                            ...
                                            {filteredChildren.length === 0 ? (
                                                ...
                                                filteredChildren.map(
                                                    (child, index) => (
                                                        <tr
                                                            key={child.id}
                                                            className={`group transition-colors ${child.today_immunization ? "bg-blue-50/60" : "hover:bg-blue-50/30"}`}
                                                        >
                                                            <td className="px-6 py-4 text-sm text-gray-500 font-medium">
                                                                {index + 1}
                                                            </td>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-bold text-gray-900">
                                            {whoModalType === "weight" &&
                                                "Standar Berat Badan WHO"}
                                            {whoModalType === "height" &&
                                                "Standar Tinggi/Panjang Badan WHO"}
                                            {whoModalType === "muac" &&
                                                "Standar Lingkar Lengan Atas WHO"}
                                            {whoModalType === "head" &&
                                                "Standar Lingkar Kepala WHO"}
                                        </h3>
                                        <p className="text-xs text-gray-500">
                                            Referensi Anak 0-5 Tahun
                                        </p>
                                    </div>
                                </div>
                                <button
                                                ...
                                                <table className="w-full text-sm">
                                                    <thead>
                                                        <tr className="border-b border-green-200">
                                                            <th className="py-2 px-3 text-left text-green-700 font-semibold">
                                                                Usia
                                                            </th>
                                                            <th className="py-2 px-3 text-center text-green-700 font-semibold">
                                                                Laki-laki
                                                            </th>
                                                            <th className="py-2 px-3 text-center text-green-700 font-semibold">
                                                                Perempuan
                                    ...
                                    <div className="space-y-4">
                                        <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                                            <h4 className="font-bold text-blue-800 mb-3">
                                                Tinggi/Panjang Badan berdasarkan
                                                Usia
                                            </h4>
                                            <div className="overflow-x-auto">
                                                <table className="w-full text-sm">
                                                    <thead>
                                                        <tr className="border-b border-blue-200">
                                                            <th className="py-2 px-3 text-left text-blue-700 font-semibold">
                                                                Usia
                                                            </th>
                                                            <th className="py-2 px-3 text-center text-blue-700 font-semibold">
                                                                Laki-laki
                                                            </th>
                                                            <th className="py-2 px-3 text-center text-blue-700 font-semibold">
                                                                Perempuan
                                    ...
                                    <div className="space-y-4">
                                        <div className="bg-teal-50 rounded-xl p-4 border border-teal-100">
                                            <h4 className="font-bold text-teal-800 mb-3">
                                                Klasifikasi Status Gizi (LILA)
                                            </h4>
                                            <div className="overflow-x-auto">
                                                <table className="w-full text-sm">
                                                    <thead>
                                                        <tr className="border-b border-teal-200">
                                                            <th className="py-2 px-3 text-left text-teal-700 font-semibold">
                                                                Kategori
                                                            </th>
                                                            <th className="py-2 px-3 text-center text-teal-700 font-semibold">
                                                                Ukuran LILA
                                                            </th>
                                                            <th className="py-2 px-3 text-center text-teal-700 font-semibold">
                                                                Status
                                        ...
                                        <div className="bg-teal-50 rounded-lg p-3 text-sm text-gray-600 border border-teal-100">
                                            ...
                                            <p className="text-xs mt-1">
                                                Pengukuran dilakukan di lengan
                                                kiri, titik tengah antara bahu
                                                dan siku
                                            </p>
                                        </div>
                                    </div>
                                ...
                                {whoModalType === "head" && (
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-600">
                                            ...
                                            <p className="text-xs mt-1">
                                                Lingkar kepala penting untuk
                                                memantau perkembangan otak anak
                                            </p>
                                        </div>
                                    </div>
                                )}
