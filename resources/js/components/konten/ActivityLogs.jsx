...
export default function ActivityLogs() {
    ...
    const fetchLogs = useCallback(
        async ({ forceRefresh = false, showLoader = false, page = 1 } = {}) => {
            const hasFilter =
                filters.action ||
                filters.model ||
                filters.user_id ||
            ...
            try {
                ...
                if (!hasFilter && page === 1) {
                    setCachedData(
                        "admin_logs",
                        { data: logsData, pagination: paginationData },
                        60,
                    );
                }
                ...
                setError(
                    err.response?.data?.message || "Gagal memuat log aktivitas",
                );
                setLoading(false);
            }
        },
    ...
    const handleExportToExcel = async () => {
        ...
        try {
            ...
            const errorMessage =
                error.message || "Terjadi kesalahan saat mengexport data";
            alert(
            ...
            <div className="flex-1 overflow-auto p-4 md:p-6 space-y-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 transition-all duration-300">
                    <div className="flex items-center justify-between mb-0 md:mb-4">
                        <button
                            ...
                            aria-label="Buka atau tutup filter log aktivitas"
                            aria-expanded={isFilterOpen}
                            aria-controls="activity-logs-filters-panel"
                        >
                            <Filter className="w-4 h-4" />
                            <span>Filter & Kontrol</span>
                            <ChevronDown
                                className={`w-4 h-4 md:hidden transition-transform ${isFilterOpen ? "rotate-180" : ""}`}
                            />
                        </button>
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                title="Auto Refresh"
                                aria-label={`Auto refresh ${autoRefresh ? "aktif" : "nonaktif"}`}
                            ...
                            <button
                                ...
                                title="Export Excel"
                                aria-label="Export log aktivitas ke Excel"
                            >
                                {isExporting ? (
                    ...
                    <div
                        id="activity-logs-filters-panel"
                        className={`grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4 mt-4 md:mt-0 ${isFilterOpen ? "grid" : "hidden md:grid"}`}
                    >
                        <div className="relative col-span-2 md:col-span-1">
                            <CustomDropdown
                                ...
                                onToggle={(val) =>
                                    setOpenDropdown(val ? "action" : null)
                                }
                            />
                        </div>
                        ...
                        <div className="relative col-span-2 md:col-span-1">
                            <CustomDropdown
                                ...
                                onToggle={(val) =>
                                    setOpenDropdown(val ? "model" : null)
                                }
                            />
                        </div>
                    ...
                    {hasActiveFilters && (
                        <div className="mt-4 flex items-center justify-between">
                            ...
                            <button
                                ...
                                aria-label="Reset semua filter log aktivitas"
                            >
                                <X className="w-4 h-4" />
                                Reset Filter
                            </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4">
                    {[
                        {
                            label: "Login",
                            count: logs.filter((l) => l.action === "login")
                                .length,
                            color: "green",
                        },
                        {
                            label: "Logout",
                            count: logs.filter((l) => l.action === "logout")
                                .length,
                            color: "gray",
                        },
                        {
                            label: "Create",
                            count: logs.filter((l) => l.action === "create")
                                .length,
                            color: "blue",
                        },
                        {
                            label: "Update",
                            count: logs.filter((l) => l.action === "update")
                                .length,
                            color: "yellow",
                        },
                        {
                            label: "Delete",
                            count: logs.filter((l) => l.action === "delete")
                                .length,
                            color: "red",
                        },
                    ].map((stat, idx) => (
                        ...
                        logs.map((log) => (
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    ...
                                    <span
                                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getActionColor(log.action)}`}
                                    >
                                        {log.action}
                                    </span>
                                </div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2 text-sm">
                                        <span
                                            className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getModelColor(log.model)}`}
                                        >
                                            {log.model || "-"}
                                        </span>
                                ...
                                {logs.length === 0 ? (
                                    ...
                                    logs.map((log) => (
                                        ...
                                        >
                                            ...
                                            <td className="py-3 px-4 text-center">
                                                <span
                                                    className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getModelColor(log.model)}`}
                                                >
                                                    {log.model || "-"}
                                                </span>
                                            </td>
                ...
                <PaginationUI
                    ...
                    onPageChange={(page) =>
                        fetchLogs({ forceRefresh: true, page })
                    }
                    label="log aktivitas"
                />
            </div>
...
function CustomDropdown({ label, value, options, onChange, isOpen, onToggle }) {
    ...
    const dropdownId = `activity-logs-dropdown-${label.toLowerCase().replace(/\s+/g, "-")}`;
                            ...
                            <div className="max-h-60 overflow-y-auto p-1">
                                {options.map((option) => (
                                    <button
                                        ...
                                        className="w-full text-left px-3 py-2 rounded-md hover:bg-blue-50 cursor-pointer flex items-center justify-between group transition-colors"
                                        aria-label={option.label}
                                    >
                                        <span
                                            className={`text-sm ${String(value) === String(option.value) ? "text-blue-600 font-medium" : "text-gray-700"}`}
                                        >
                                            {option.label}
                                        </span>
                                        {String(value) ===
...
function CustomDatePicker({ label, value, onChange }) {
    ...
    const datePickerId = `activity-logs-date-${label.toLowerCase().replace(/\s+/g, "-")}`;
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className="font-semibold text-gray-800 text-sm">
                                    {pickerDate.toLocaleDateString("id-ID", {
                                        month: "long",
                                        year: "numeric",
                                    })}
                                </span>
