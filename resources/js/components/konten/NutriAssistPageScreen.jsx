...
const DAILY_REQUEST_STORAGE_KEY = "nutriassist_daily_usage";
...
export default function NutriAssistPage() {
    ...
    const [selectedChildHasWeighing, setSelectedChildHasWeighing] =
        useState(true);
    ...
    const syncDailyCounter = () => {
        ...
        try {
            const stored = localStorage.getItem(DAILY_REQUEST_STORAGE_KEY);
            if (!stored) {
                localStorage.setItem(
                    DAILY_REQUEST_STORAGE_KEY,
    ...
    // Check if selected child has weighing data
    useEffect(() => {
        const checkWeighingData = async () => {
            if (!selectedChildId || children.length === 0) {
                setSelectedChildHasWeighing(true);
                return;
            }
    ...
    const handleSubmit = async (e) => {
        ...
        if (dailyRequestCount >= DAILY_REQUEST_LIMIT) {
            setError(
                `Batas harian Nutri-Assist sudah tercapai (${DAILY_REQUEST_LIMIT} request/hari). Coba lagi besok.`,
            );
            return;
        }
        ...
        try {
            ...
            const response = await api.post(
                `/parent/children/${selectedChildId}/nutri-assist`,
                payload,
            );
            ...
            if (newRecommendations?.recommendations) {
                newRecommendations.recommendations =
                    newRecommendations.recommendations.map((rec, index) => {
                        ...
                        if (
                            ...
                            const recIngredientsLower = rec.ingredients.map(
                                (i) => i.toLowerCase(),
                            );
                            const providedIngredientsLower =
                                payload.ingredients.map((i) => i.toLowerCase());
                            const matchedCount = recIngredientsLower.filter(
                                (ing) =>
                                    providedIngredientsLower.some(
                                        (provided) =>
                                            ing.includes(provided) ||
                                            provided.includes(ing),
                                    ),
                            ).length;
                            matchPercentage =
                                (matchedCount / rec.ingredients.length) * 100;
                        }
                        ...
                        return {
                            ...
                            calories:
                                rec.nutrition?.calories || rec.calories || 0,
            ...
            setDailyRequestCount(nextCount);
            localStorage.setItem(
                DAILY_REQUEST_STORAGE_KEY,
                JSON.stringify({
                    date: getTodayString(),
            ...
            if (err.response?.status === 403) {
                setError(
                    "Anda tidak memiliki akses untuk mendapatkan rekomendasi untuk anak ini.",
                );
            } else if (err.response?.status === 404) {
                setError("Data anak tidak ditemukan.");
            } else if (
                err.response?.status === 422 &&
                err.response?.data?.requires_weighing
            ) {
    ...
    const handleAddToJournal = (rec, index) => {
        ...
        navigate("/dashboard/jurnal-makan", {
            state: { prefilledMeal: prefilledData },
        });
    };
        ...
        <div className="flex flex-col flex-1 w-full h-full overflow-hidden bg-gradient-to-br from-blue-50 via-indigo-50/50 to-purple-50">
            ...
            <div className="flex-1 w-full overflow-y-auto custom-scrollbar no-scrollbar md:scrollbar-auto">
                <div className="w-full p-4 md:p-8 lg:p-10">
                    <div className="mb-8">
                        <div className="relative overflow-hidden bg-white/60 backdrop-blur-md rounded-2xl p-6 border border-white/60 shadow-sm group hover:shadow-md transition-all duration-300">
                            <div className="absolute -right-6 -top-6 w-24 h-24 bg-gradient-to-br from-blue-500/10 to-purple-500/10 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500" />

                            <div className="relative flex flex-col md:flex-row gap-4 items-start md:items-center">
                                <div className="p-3 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl shadow-lg shadow-blue-500/20 text-white transform group-hover:scale-110 transition-transform duration-300">
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="px-2 py-0.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-[10px] font-bold uppercase tracking-wider rounded-full shadow-sm">
                                            AI Powered
                                        </span>
                                    </div>
                                    <p className="text-gray-600 text-sm md:text-base max-w-2xl leading-relaxed">
                                        Asisten pintar yang membantu Anda
                                        meracik menu bergizi dari bahan yang
                                        tersedia di rumah. Hemat waktu, kurangi
                                        limbah makanan, dan pastikan nutrisi si
                                        kecil terpenuhi.
                                    </p>
                                    <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-blue-200 bg-blue-50 text-xs font-semibold text-blue-700">
                                        <BookOpen className="w-3.5 h-3.5" />
                                        Request Hari Ini: {dailyRequestCount}/
                                        {DAILY_REQUEST_LIMIT}
                                    </div>
                                </div>
                            </div>
                    ...
                    {children.length === 0 ? (
                        ...
                        >
                            ...
                            <p className="text-gray-500 max-w-md mb-8">
                                Silakan daftarkan anak Anda terlebih dahulu di
                                Posyandu untuk mulai menggunakan fitur
                                Nutri-Assist.
                            </p>
                        </motion.div>
                    ) : (
                                    ...
                                    <div className="flex items-center gap-3 mb-6 relative z-10">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500">
                                                Isi detail untuk rekomendasi
                                            </p>
                                        </div>
                                    </div>

                                    {/* Warning Banner for No Weighing Data */}
                                    {!selectedChildHasWeighing &&
                                        selectedChildId && (
                                            <motion.div
                                                initial={{
                                                    opacity: 0,
                                            ...
                                            >
                                                <div className="bg-amber-50/90 backdrop-blur-sm border-2 border-amber-200 rounded-2xl p-4">
                                                            ...
                                                            0 ? (
                                                                ...
                                                                children.map(
                                                                    (child) => (
                                                                        ...
                                                                        >
                                                                            <span
                                                                                className={`text-sm ${selectedChildId === child.id.toString() ? "text-blue-700 font-semibold" : "text-gray-700 font-medium group-hover:text-blue-700"}`}
                                                                            >
                                                                                {
                                                                                    child.full_name
                                                                                }{" "}
                                ...
                                {!recommendations ? (
                                    <div className="h-full min-h-[400px] flex flex-col items-center justify-center text-center p-8 bg-white/40 backdrop-blur-md rounded-3xl border border-white/40 border-dashed">
                                        ...
                                        <p className="text-gray-500 max-w-sm">
                                            ...
                                            miliki di panel sebelah kiri, dan
                                            kami akan menyarankan menu terbaik
                                            untuk si kecil.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="space-y-6">
                                        ...
                                        {recommendations.recommendations
                                            ...
                                            <motion.div
                                                ...
                                                className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 border border-white/50 shadow-lg space-y-4"
                                            >
                                                <div className="text-center mb-6">
                                                    ...
                                                    <h4 className="text-lg font-semibold text-gray-800 mb-2">
                                                        Saran Nutrisi untuk Anda
                                                    </h4>
                                                    <p className="text-gray-500 text-sm">
                                                        Tidak ada rekomendasi
                                                        menu spesifik, tetapi
                                                        kami memiliki saran
                                                        penting untuk Anda.
                                                    </p>
                                                </div>
                                                                                    ...
                                                                                    <div className="grid grid-cols-2 gap-3">
                                                                                        ...
                                                                                        {rec.meal_type && (
                                                                                            <div className="bg-pink-50 p-3 rounded-lg border border-pink-100">
                                                                                                ...
                                                                                                <div className="text-sm font-bold text-pink-700 capitalize">
                                                                                                    {rec.meal_type.replace(
                                                                                                        /_/g,
                                                                                                        " ",
                                                                                                    )}
                                                                                                </div>
