    ...
    const handleShareChildData = async () => {
        ...
        try {
            const response = await api.get(
                `/parent/consultations/${id}/child-data`,
            );
...
Catatan: ${data.notes || "-"}`;
    ...
    const fetchConsultation = async (consultationId, silent = false) => {
        try {
            ...
            const response = await api.get(
                `/parent/consultations/${consultationId}`,
            );
    ...
    const sendMessage = async (e) => {
        ...
        try {
            ...
            const response = await api.post(
                `/parent/consultations/${id}/messages`,
                formData,
                {
                    headers: {
                    ...
                    <AnimatePresence initial={false}>
                        {consultation.messages.map((message, index) => {
                            ...
                            return (
                                ...
                                >
                                    <div
                                        className={`flex flex-col max-w-[85%] sm:max-w-[70%] ${isParent ? "items-end" : "items-start"}`}
                                    >
                                        {message.message &&
                                        ...
                                        <div
                                            className={`flex items-center gap-1 mt-1.5 text-[10px] font-medium ${isParent ? "text-blue-900/40" : "text-slate-400"}`}
                                        >
