...
export default function CreateConsultation() {
    ...
    const selectedKader = kaders.find(
        (kader) => kader.id === parseInt(selectedKaderId, 10),
    );
    ...
    useEffect(() => {
        if (selectedChildId && children.length > 0 && kaders.length > 0) {
            ...
            if (child && child.posyandu_id) {
                ...
                const recommendedKader = kaders.find(
                    (k) => k.posyandu_id === child.posyandu_id,
                );
                if (recommendedKader) {
    ...
    const getEstimatedResponseTime = (kader) => {
        if (!kader) return "-";
        return kader.is_online ? "5-15 menit" : "< 24 jam";
    };

    const kaderSpecialties = ["Gizi Balita", "MPASI", "Tumbuh Kembang"];
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <p className="mt-2 text-xs text-slate-500">
                                    Jelaskan secara singkat apa yang ingin Anda
                                    tanyakan.
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="mt-2 text-xs text-slate-500 flex items-center gap-1">
                                    ...
                                    Kader akan otomatis disesuaikan dengan
                                    posyandu anak, namun Anda dapat mengubahnya.
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) =>
                                            setSelectedKaderId(e.target.value)
                                        }
                                        className="w-full pl-12 pr-10 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-slate-800 appearance-none"
                                        disabled={submitting}
                                    >
                                ...
                                <p className="mt-2 text-xs text-slate-500">
                                    Pilih kader yang ingin Anda ajak
                                    berkonsultasi.
                                </p>
                                        ...
                                        <div className="flex items-center justify-between gap-3">
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-600">
                                                    {selectedKader.posyandu?.name
                                                        ? `Posyandu ${selectedKader.posyandu.name}`
                                                        : "Kader Posyandu"}
                                                </p>
                                            </div>
                                            <span
                                                className={`text-xs px-2 py-1 rounded-full font-medium ${
                                                    selectedKader.is_online
